...
/// struct for typed errors of method [`assign_project_user_role`]
...
/// struct for typed errors of method [`list_project_user_role_assignments`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// struct for typed errors of method [`retrieve_project_user_role`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
/// struct for typed errors of method [`unassign_project_user_role`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn retrieve_project_user_role(
    ...
    user_id: &str,
    role_id: &str,
) -> Result<models::AssignedRoleDetails, Error<RetrieveProjectUserRoleError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_path_project_id = project_id;
    let p_path_user_id = user_id;
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        let content = resp.text().await?;
        let entity: Option<RetrieveProjectUserRoleError> = serde_json::from_str(&content).ok();
        Err(Error::ResponseError(ResponseContent {
            status,
            content,
            entity,
