...
#[bon::builder]
pub async fn create_transcription(configuration: &configuration::Configuration, file: std::path::PathBuf, model: &str, language: Option<&str>, prompt: Option<&str>, response_format: Option<models::AudioResponseFormat>, temperature: Option<f64>, stream: Option<bool>, chunking_strategy: Option<models::TranscriptionChunkingStrategy>, timestamp_granularities: Option<Vec<String>>, include: Option<Vec<models::TranscriptionInclude>>) -> Result<models::CreateTranscription200Response, Error<CreateTranscriptionError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    ...
    if let Some(param_value) = p_form_timestamp_granularities {
        multipart_form = multipart_form.text("timestamp_granularities", param_value.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string());
    }
    if let Some(param_value) = p_form_include {
        multipart_form = multipart_form.text("include", param_value.into_iter().map(|p| p.to_string()).collect::<Vec<String>>().join(",").to_string());
    }
...
#[bon::builder]
pub async fn create_translation(configuration: &configuration::Configuration, file: std::path::PathBuf, model: &str, prompt: Option<&str>, response_format: Option<&str>, temperature: Option<f64>) -> Result<models::CreateTranslation200Response, Error<CreateTranslationError>> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_form_file = file;
    let p_form_model = model;
    let p_form_prompt = prompt;
