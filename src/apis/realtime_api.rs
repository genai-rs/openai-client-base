...
/// struct for typed errors of method [`create_realtime_translation_client_secret`]
#[derive(Debug, Clone, Serialize, Deserialize)]
#[serde(untagged)]
...
pub async fn create_realtime_translation_client_secret(
    configuration: &configuration::Configuration,
    realtime_translation_client_secret_create_request: models::RealtimeTranslationClientSecretCreateRequest,
) -> Result<
    models::RealtimeTranslationClientSecretCreateResponse,
    Error<CreateRealtimeTranslationClientSecretError>,
> {
    // add a prefix to parameters to efficiently prevent name collisions
    let p_body_realtime_translation_client_secret_create_request =
        realtime_translation_client_secret_create_request;
    ...
    req_builder = req_builder.json(&p_body_realtime_translation_client_secret_create_request);
    ...
    if !status.is_client_error() && !status.is_server_error() {
        ...
        match content_type {
            ContentType::Json => serde_json::from_str(&content).map_err(Error::from),
            ContentType::Text => return Err(Error::from(serde_json::Error::custom("Received `text/plain` content type response that cannot be converted to `models::RealtimeTranslationClientSecretCreateResponse`"))),
            ContentType::Unsupported(unknown_type) => return Err(Error::from(serde_json::Error::custom(format!("Received `{unknown_type}` content type response that cannot be converted to `models::RealtimeTranslationClientSecretCreateResponse`")))),
        }
    } else {
        let content = resp.text().await?;
