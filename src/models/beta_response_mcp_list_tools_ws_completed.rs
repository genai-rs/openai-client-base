/*
 * OpenAI API
 *
 * The OpenAI REST API. Please see https://platform.openai.com/docs/api-reference for more details.
 *
 * The version of the OpenAPI document: 2.3.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// BetaResponseMcpListToolsWsCompleted : Emitted when the list of available MCP tools has been successfully retrieved.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize, bon::Builder)]
pub struct BetaResponseMcpListToolsWsCompleted {
    #[serde(rename = "agent", skip_serializing_if = "Option::is_none")]
    pub agent: Option<Box<models::BetaAgentTag>>,
    /// The type of the event. Always 'response.mcp_list_tools.completed'.
    #[serde(rename = "type")]
    pub r#type: Type,
    /// The ID of the MCP tool call item that produced this output.
    #[serde(rename = "item_id")]
    pub item_id: String,
    /// The index of the output item that was processed.
    #[serde(rename = "output_index")]
    pub output_index: i32,
    /// The sequence number of this event.
    #[serde(rename = "sequence_number")]
    pub sequence_number: i32,
    /// The WebSocket lane that emitted this event. This field is present when the originating `response.create` event supplied a `stream_id`.
    #[serde(rename = "stream_id", skip_serializing_if = "Option::is_none")]
    pub stream_id: Option<String>,
}

impl BetaResponseMcpListToolsWsCompleted {
    /// Emitted when the list of available MCP tools has been successfully retrieved.
    pub fn new(
        r#type: Type,
        item_id: String,
        output_index: i32,
        sequence_number: i32,
    ) -> BetaResponseMcpListToolsWsCompleted {
        BetaResponseMcpListToolsWsCompleted {
            agent: None,
            r#type,
            item_id,
            output_index,
            sequence_number,
            stream_id: None,
        }
    }
}
/// The type of the event. Always 'response.mcp_list_tools.completed'.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "response.mcp_list_tools.completed")]
    ResponseMcpListToolsCompleted,
}

impl Default for Type {
    fn default() -> Type {
        Self::ResponseMcpListToolsCompleted
    }
}

impl std::fmt::Display for BetaResponseMcpListToolsWsCompleted {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match serde_json::to_string(self) {
            Ok(s) => write!(f, "{}", s),
            Err(_) => Err(std::fmt::Error),
        }
    }
}
