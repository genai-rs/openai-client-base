...
pub enum TokenCountsBodyInput {
    Text(String),
    Arrayofinputitems(Vec<models::InputItem>),
    Null,
}
...
impl TokenCountsBodyInput {
    ...
    pub fn new_arrayofinputitems(items: Vec<models::InputItem>) -> Self {
        Self::Arrayofinputitems(items)
    }
}
...
impl std::fmt::Display for TokenCountsBodyInput {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match self {
            ...
            TokenCountsBodyInput::Arrayofinputitems(value) => match serde_json::to_string(value) {
                Ok(s) => write!(f, "{}", s),
