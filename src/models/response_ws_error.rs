...
/// ResponseWsError : Emitted when an error occurs while processing a Responses WebSocket request.
...
pub struct ResponseWsError {
    ...
    #[serde(rename = "error")]
    pub error: Box<models::ErrorPayload>,
...
impl ResponseWsError {
    /// Emitted when an error occurs while processing a Responses WebSocket request.
    pub fn new(r#type: Type, error: models::ErrorPayload) -> ResponseWsError {
        ResponseWsError {
            r#type,
            status: None,
...
impl std::fmt::Display for ResponseWsError {
