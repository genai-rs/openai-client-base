/*
 * OpenAI API
 *
 * The OpenAI REST API. Please see https://platform.openai.com/docs/api-reference for more details.
 *
 * The version of the OpenAPI document: 2.3.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// ResponseShellCallCommandDoneStreamingEvent : A streaming event that indicated a shell command was completed.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize, bon::Builder)]
pub struct ResponseShellCallCommandDoneStreamingEvent {
    /// The type of the event, always `response.shell_call_command.done`.
    #[serde(rename = "type")]
    pub r#type: Type,
    /// The sequence number of the event that was emitted.
    #[serde(rename = "sequence_number")]
    pub sequence_number: i32,
    /// The index of the output item that was updated.
    #[serde(rename = "output_index")]
    pub output_index: i32,
    /// The index of the shell command that was completed.
    #[serde(rename = "command_index")]
    pub command_index: i32,
    /// The final shell command that was emitted.
    #[serde(rename = "command")]
    pub command: String,
}

impl ResponseShellCallCommandDoneStreamingEvent {
    /// A streaming event that indicated a shell command was completed.
    pub fn new(
        r#type: Type,
        sequence_number: i32,
        output_index: i32,
        command_index: i32,
        command: String,
    ) -> ResponseShellCallCommandDoneStreamingEvent {
        ResponseShellCallCommandDoneStreamingEvent {
            r#type,
            sequence_number,
            output_index,
            command_index,
            command,
        }
    }
}
/// The type of the event, always `response.shell_call_command.done`.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "response.shell_call_command.done")]
    ResponseShellCallCommandDone,
}

impl Default for Type {
    fn default() -> Type {
        Self::ResponseShellCallCommandDone
    }
}

impl std::fmt::Display for ResponseShellCallCommandDoneStreamingEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match serde_json::to_string(self) {
            Ok(s) => write!(f, "{}", s),
            Err(_) => Err(std::fmt::Error),
        }
    }
}
