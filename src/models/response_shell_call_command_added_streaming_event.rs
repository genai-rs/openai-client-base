/*
 * OpenAI API
 *
 * The OpenAI REST API. Please see https://platform.openai.com/docs/api-reference for more details.
 *
 * The version of the OpenAPI document: 2.3.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// ResponseShellCallCommandAddedStreamingEvent : A streaming event that indicated a shell command was added to a tool call.
#[derive(Clone, Default, Debug, PartialEq, Serialize, Deserialize, bon::Builder)]
pub struct ResponseShellCallCommandAddedStreamingEvent {
    /// The type of the event, always `response.shell_call_command.added`.
    #[serde(rename = "type")]
    pub r#type: Type,
    /// The sequence number of the event that was emitted.
    #[serde(rename = "sequence_number")]
    pub sequence_number: i32,
    /// The index of the output item that was updated.
    #[serde(rename = "output_index")]
    pub output_index: i32,
    /// The index of the shell command that was added.
    #[serde(rename = "command_index")]
    pub command_index: i32,
    /// The shell command that was added.
    #[serde(rename = "command")]
    pub command: String,
}

impl ResponseShellCallCommandAddedStreamingEvent {
    /// A streaming event that indicated a shell command was added to a tool call.
    pub fn new(
        r#type: Type,
        sequence_number: i32,
        output_index: i32,
        command_index: i32,
        command: String,
    ) -> ResponseShellCallCommandAddedStreamingEvent {
        ResponseShellCallCommandAddedStreamingEvent {
            r#type,
            sequence_number,
            output_index,
            command_index,
            command,
        }
    }
}
/// The type of the event, always `response.shell_call_command.added`.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Type {
    #[serde(rename = "response.shell_call_command.added")]
    ResponseShellCallCommandAdded,
}

impl Default for Type {
    fn default() -> Type {
        Self::ResponseShellCallCommandAdded
    }
}

impl std::fmt::Display for ResponseShellCallCommandAddedStreamingEvent {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match serde_json::to_string(self) {
            Ok(s) => write!(f, "{}", s),
            Err(_) => Err(std::fmt::Error),
        }
    }
}
