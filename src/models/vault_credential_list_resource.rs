/*
 * OpenAI API
 *
 * The OpenAI REST API. Please see https://platform.openai.com/docs/api-reference for more details.
 *
 * The version of the OpenAPI document: 2.3.0
 *
 * Generated by: https://openapi-generator.tech
 */

use crate::models;
use serde::{Deserialize, Serialize};

/// VaultCredentialListResource : A page of Agents API resources, with IDs for retrieving additional pages.
#[derive(Clone, Debug, PartialEq, Serialize, Deserialize, bon::Builder)]
pub struct VaultCredentialListResource {
    /// The object type, which is always `list`.
    #[serde(rename = "object")]
    pub object: Object,
    /// The resources returned in this page, in the requested sort order.
    #[serde(rename = "data")]
    pub data: Vec<models::VaultCredentialResource>,
    /// The ID of the first resource in `data`, or `null` if the page is empty.
    #[serde(rename = "first_id", deserialize_with = "Option::deserialize")]
    pub first_id: Option<String>,
    /// The ID of the last resource in `data`, or `null` if the page is empty. Pass this as `after` with the same order and filters.
    #[serde(rename = "last_id", deserialize_with = "Option::deserialize")]
    pub last_id: Option<String>,
    /// Whether there are more resources to retrieve after this page.
    #[serde(rename = "has_more")]
    pub has_more: bool,
}

impl VaultCredentialListResource {
    /// A page of Agents API resources, with IDs for retrieving additional pages.
    pub fn new(
        object: Object,
        data: Vec<models::VaultCredentialResource>,
        first_id: Option<String>,
        last_id: Option<String>,
        has_more: bool,
    ) -> VaultCredentialListResource {
        VaultCredentialListResource {
            object,
            data,
            first_id,
            last_id,
            has_more,
        }
    }
}
/// The object type, which is always `list`.
#[derive(Clone, Copy, Debug, Eq, PartialEq, Ord, PartialOrd, Hash, Serialize, Deserialize)]
pub enum Object {
    #[serde(rename = "list")]
    List,
}

impl Default for Object {
    fn default() -> Object {
        Self::List
    }
}

impl std::fmt::Display for VaultCredentialListResource {
    fn fmt(&self, f: &mut std::fmt::Formatter<'_>) -> std::fmt::Result {
        match serde_json::to_string(self) {
            Ok(s) => write!(f, "{}", s),
            Err(_) => Err(std::fmt::Error),
        }
    }
}
