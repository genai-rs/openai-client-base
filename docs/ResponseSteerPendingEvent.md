...
**reason** | [**models::ResponseSteerPendingReason**](ResponseSteerPendingReason.md) |  | 
**required_input** | [**Vec<models::ResponseSteerRequiredInput>**](ResponseSteerRequiredInput.md) | Input stubs identifying outstanding client-owned tool results or approval decisions. Each stub contains identifying fields only; the client supplies the result before including it in `response.create`.  | 
